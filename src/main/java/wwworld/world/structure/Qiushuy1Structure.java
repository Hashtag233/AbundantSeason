...
import wwworld.procedures.Yingshu1FuJiaDeShengChengTiaoJianProcedure;
...
public class Qiushuy1Structure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 100000) {
						...
						for (int a = 0; a < count; a++) {
							...
							if (blockAt.getBlock() == QiuzhicaofangkuaiBlock.block)
								blockCriteria = true;
							if (!blockCriteria)
								continue;
							...
							int y = spawnTo.getY();
							int z = spawnTo.getZ();
							if (!Yingshu1FuJiaDeShengChengTiaoJianProcedure.executeProcedure(Stream
									.of(new AbstractMap.SimpleEntry<>("world", world), new AbstractMap.SimpleEntry<>("x", x),
											new AbstractMap.SimpleEntry<>("y", y), new AbstractMap.SimpleEntry<>("z", z))
									.collect(HashMap::new, (_m, _e) -> _m.put(_e.getKey(), _e.getValue()), Map::putAll)))
								continue;
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("wwworld", "qiushuy1"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("qiushuy_1"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("wwworld:qiushuy_1"), configuredFeature);
		}
	}
