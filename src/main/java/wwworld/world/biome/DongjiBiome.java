...
public class DongjiBiome extends WwworldModElements.ModElement {
	public static Biome biome;

	public DongjiBiome(WwworldModElements instance) {
		super(instance, 342);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-3545094).setWaterColor(-1).setWaterFogColor(-1)
						.withSkyColor(-3545094).withFoliageColor(-3342337).withGrassColor(-1)
						.setMusic(new BackgroundMusicSelector(
								(net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("wwworld:dong")), 12000,
								24000, true))
						.setParticle(new ParticleEffectAmbience(XuehuaParticle.particle, 0.025f)).build();
				BiomeGenerationSettings.Builder biomeGenerationSettings = new BiomeGenerationSettings.Builder()
						.withSurfaceBuilder(SurfaceBuilder.DEFAULT.func_242929_a(new SurfaceBuilderConfig(DongzhimuBlock.block.getDefaultState(),
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.NONE).category(Biome.Category.NONE).depth(0f).scale(1.5f).temperature(2f)
						.downfall(0f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("wwworld:han_chao_lin"));
			}
		}
	}
