...
import wwworld.block.XiazhidifangkuaiBlock;
...
public class HuijinlinBiome extends WwworldModElements.ModElement {
	public static Biome biome;

	public HuijinlinBiome(WwworldModElements instance) {
		super(instance, 403);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-9277328).setWaterColor(-10066330).setWaterFogColor(-10066330)
						.withSkyColor(-9277328).withFoliageColor(-16777216).withGrassColor(-10066330)
						.setMusic(new BackgroundMusicSelector(
								...
								24000, true))
						.setParticle(new ParticleEffectAmbience(HuiseliziParticle.particle, 0.05f)).build();
				BiomeGenerationSettings.Builder biomeGenerationSettings = new BiomeGenerationSettings.Builder().withSurfaceBuilder(
						SurfaceBuilder.DEFAULT.func_242929_a(new SurfaceBuilderConfig(XiazhidifangkuaiBlock.block.getDefaultState(),
								XiazhidifangkuaiBlock.block.getDefaultState(), XiazhidifangkuaiBlock.block.getDefaultState())));
				DefaultBiomeFeatures.withFrozenTopLayer(biomeGenerationSettings);
				DefaultBiomeFeatures.withCavesAndCanyons(biomeGenerationSettings);
				DefaultBiomeFeatures.withLavaAndWaterLakes(biomeGenerationSettings);
				MobSpawnInfo.Builder mobSpawnInfo = new MobSpawnInfo.Builder().isValidSpawnBiomeForPlayer();
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(0f).scale(0f).temperature(2f)
						.downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("wwworld:hui_jin_lin"));
			}
		}
	}
