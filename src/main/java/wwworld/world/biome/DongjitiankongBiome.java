...
import wwworld.block.YunkuaiBlock;
...
public class DongjitiankongBiome extends WwworldModElements.ModElement {
	public static Biome biome;

	public DongjitiankongBiome(WwworldModElements instance) {
		super(instance, 434);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-3026479).setWaterColor(-1).setWaterFogColor(-1)
						.withSkyColor(-3026479).withFoliageColor(-1).withGrassColor(-1)
				...
				BiomeGenerationSettings.Builder biomeGenerationSettings = new BiomeGenerationSettings.Builder()
						.withSurfaceBuilder(SurfaceBuilder.DEFAULT.func_242929_a(new SurfaceBuilderConfig(YunkuaiBlock.block.getDefaultState(),
								YunkuaiBlock.block.getDefaultState(), YunkuaiBlock.block.getDefaultState())));
				DefaultBiomeFeatures.withFrozenTopLayer(biomeGenerationSettings);
				DefaultBiomeFeatures.withCavesAndCanyons(biomeGenerationSettings);
				DefaultBiomeFeatures.withOverworldOres(biomeGenerationSettings);
				MobSpawnInfo.Builder mobSpawnInfo = new MobSpawnInfo.Builder().isValidSpawnBiomeForPlayer();
				biome = new Biome.Builder().precipitation(Biome.RainType.NONE).category(Biome.Category.NONE).depth(3f).scale(0f).temperature(2f)
						.downfall(0f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("wwworld:zhui_luo_tian_kong"));
			}
		}
	}
