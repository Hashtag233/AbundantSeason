...
import wwworld.procedures.AncaoFuJiaDeShengChengTiaoJianProcedure;
...
public class AncaoBlock extends WwworldModElements.ModElement {
	@ObjectHolder("wwworld:ancao")
	public static final Block block = null;

	public AncaoBlock(WwworldModElements instance) {
		super(instance, 116);
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
			feature = new RandomPatchFeature(BlockClusterFeatureConfig.field_236587_a_) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, BlockClusterFeatureConfig config) {
					...
					int y = pos.getY();
					int z = pos.getZ();
					if (!AncaoFuJiaDeShengChengTiaoJianProcedure.executeProcedure(Stream
			...
			configuredFeature = feature.withConfiguration(
					(new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(block.getDefaultState()), new SimpleBlockPlacer())).tries(64)
							.build())
					.withPlacement(Placement.COUNT_NOISE.configure(new NoiseDependant(-0.8, 0, 30)));
			event.getRegistry().register(feature.setRegistryName("ancao"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("wwworld:ancao"), configuredFeature);
		}
	}
	...
	public static class BlockCustomFlower extends FlowerBlock {
		public BlockCustomFlower() {
			super(Effects.SPEED, 0, Block.Properties.create(Material.PLANTS).doesNotBlockMovement().sound(SoundType.PLANT)
					.hardnessAndResistance(0f, 0f).setLightLevel(s -> 0));
			setRegistryName("ancao");
		}
